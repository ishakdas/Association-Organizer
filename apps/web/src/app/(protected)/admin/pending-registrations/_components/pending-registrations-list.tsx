...
import { Clock, Phone, MessageSquare, MapPin, X, RefreshCw, CheckCircle2, RotateCcw, Plus, XCircle } from 'lucide-react';
...
const REJECTED_KEY = ['rejected-registrations'] as const;
...
export function PendingRegistrationsList({
  ...
  const { data: rejectedData } = useQuery({
    ...
    queryFn: async () => listRejectedRegistrations(await getToken()),
      ...
      {tab === 'rejected' && (
        <>
          {!rejectedData || rejectedData.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-muted/30 py-16 text-center">
              <XCircle className="mb-3 h-8 w-8 text-muted-foreground" />
              <p className="text-sm font-medium text-foreground">Reddedilmiş başvuru yok</p>
              <p className="mt-1 text-[13px] text-muted-foreground">
                Reddedilen başvurular burada görünecek; istenirse tekrar onaylanabilir.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {rejectedData.map((reg) => (
                <RejectedCard
                  key={reg.id}
                  registration={reg}
                  onApprove={() => setApproveTarget(reg)}
