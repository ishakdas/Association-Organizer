...
const MANAGER = {
  ...
  systemRole: null,
  memberships: [
    { id: 'mem-mgr', associationId: ASSOC, role: 'ASSOCIATION_MANAGER', isActive: true },
  ],
} as any;
const SECRETARY = {
  id: 'sec-1',
  systemRole: null,
  memberships: [
    { id: 'mem-sec', associationId: ASSOC, role: 'ASSOCIATION_SECRETARY', isActive: true },
  ],
} as any;
const MEMBER = {
  id: 'mem-1',
  systemRole: null,
  memberships: [
    { id: 'mem-mem', associationId: ASSOC, role: 'ASSOCIATION_MEMBER', isActive: true },
  ],
} as any;
const FINANCE_PERM_USER = {
  ...
  systemRole: null,
  memberships: [
    { id: 'mem-fin', associationId: ASSOC, role: 'ASSOCIATION_MEMBER', isActive: true },
  ],
} as any;
...
describe('FinanceService', () => {
  ...
  describe('recordEventExpense', () => {
    it('records expense and creates transaction', async () => {
      ...
      const result = await service.recordEventExpense(
        ASSOC,
        'evt-1',
        {
          expenseAmount: 15000,
  ...
  describe('bulkFeePayment', () => {
    it('creates multiple fee payments and returns result', async () => {
      ...
      prisma.associationMembership.findFirst
        .mockResolvedValueOnce({
          id: 'mem-1',
          user: { id: 'u1', fullName: 'Ahmet Yılmaz' },
        } as never)
        .mockResolvedValueOnce({
          id: 'mem-2',
          user: { id: 'u2', fullName: 'Fatma Demir' },
        } as never);
      ...
      const result = await service.bulkFeePayment(
        ASSOC,
        {
          payments: [
    ...
    it('skips duplicate payments for same month', async () => {
      ...
      const result = await service.bulkFeePayment(
        ASSOC,
        {
          payments: [{ membershipId: 'mem-1', amountInKurus: 50000, month: '2026-05' }],
        },
        MANAGER,
      );
    ...
    it('skips inactive memberships', async () => {
      ...
      const result = await service.bulkFeePayment(
        ASSOC,
        {
          payments: [{ membershipId: 'mem-inactive', amountInKurus: 50000, month: '2026-05' }],
        },
        MANAGER,
      );
